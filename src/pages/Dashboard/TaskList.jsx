...
const TaskListTable = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await fetch("http://localhost:9001/api/taskList/get");
        if (!response.ok) {
  ...
  useEffect(() => {
    let filtered = taskListData.filter((record) => {
      return (
  ...
  }, [searchTerm, month, year, taskListData]);
            ...
            {taskListData.length > 0 ? (
              taskListData.map((record, index) => (
                <tr key={index}>
                  ...
                  <td className="text-center">{record.note}</td>
